...
iotable_get <- function ( source = "germany_1990", geo = "DE",
                          year = 1990, unit = "MIO_EUR", labelling = "iotables" ) {  
  ...
  iotables_label_r =NULL; iotables_label_c = NULL; prod_na = NULL; induse = NULL
...
##Veryifing year and country name input ---
  ...
  if ( source %in% prod_ind ) {
    iotable_labelled <- iotable %>%
      ...
      left_join (., metadata_cols, by = c("prod_na", "prod_na_lab"))  %>%
      left_join ( ., metadata_rows, by = c("induse", "induse_lab") ) %>%
  ...
  if ( labelling == "iotables") {
    iotable_labelled <- iotable_labelled %>%
      arrange ( iotables_label_r, iotables_label_c) %>%
