...
#'
#' @examples
#' ids <- c("p1","p2"); fin <- c("hfce","gfcf"); pri <- c("va","taxes")
...
iotable_create_stacked <- function(q1, q2, q3,
  ...
  n <- length(q1_id); k <- length(final_use_id); m <- length(primary_id)
  ...
  if (isTRUE(fence_q1)) {
    ...
    fence_col <- matrix(colSums(q1), nrow = n, ncol = 1,
                        dimnames = list(q1_id, fence_col_label))
    upper <- cbind(q1, fence_col, q2)
    lower <- cbind(q3, matrix(NA_real_, nrow = m, ncol = 1,
                              dimnames = list(primary_id,
                                              fence_col_label)),
                   q4)
    fence_col_idx <- match(fence_col_label,
                           colnames(cbind(q1, fence_col, q2)))
    # total row = rowSums(Z), placed under industries (before Q3)
    fence_row <- matrix(rowSums(q1), nrow = 1, byrow = TRUE,
                        dimnames = list(fence_row_label, q1_id))
    fence_row_Q4 <- matrix(NA_real_, nrow = 1, ncol = k,
