...
#' intermediate-use block (Quadrant 1), the value-added block (Quadrant 3),
#' and the final-use and balancing items (Quadrant 2/50).
#'
#' @details
#' The `ind_use` vocabulary corresponds to the official Eurostat SDMX codelist
#' *“Industry uses”*.
#' It is used by the [iotables] package to order the **column dimension**
#' of industry-by-industry tables (datasets such as `naio_10_cp1750`).
#'
#' Codes follow **NACE Rev. 2** for industries and **ESA 2010** for accounting
#' aggregates.
#' Each entry includes its quadrant classification and an ordinal numeric
#' sequence for reproducible column ordering when reshaping long-form data.
#'
#' The vocabulary also includes recent Eurostat additions introduced in 2023
#' (*ITTM* – International trade and transport margins, *MCH* – Merchanting,
#' and *OP* – Purchases by residents abroad).
