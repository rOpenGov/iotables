...
```{r emmissioncoeffs}
emmission_coeffs <- output_coefficients_create(data_table = emissions_de, 
                                               output = output_bp)
```
```{r emmissioncoeffsprint}
round_table(emmission_coeffs,4)
```
...
emission_multipliers <- cbind (
  ...
  do.call( rbind, 
           lapply ( 1:nrow(coeffs), function(x) equation_solve (coeffs[x, ], I_de) )
           ) %>% as.data.frame()
  )

round_table(emission_multipliers, 4)
```
