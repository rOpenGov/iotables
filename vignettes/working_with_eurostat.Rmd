...
# The following code can replicate the retrieval.  we have saved the results into the 
# internal naio_10_product_x_product.rda file of the iotables package. Because of the file 
# sizes invoved, this may take several minutes.
...
emp_sk <- employment_get(geo = "SK", year = "2017", 
                         sex = "Total", age = "Y_GE15", labelling = "prod_na",
                         data_directory = not_included_directory,
                         force_download = TRUE)
```
...
To make demand-driven Leontief input-output system work we need to create some special matrixes. If you work in the supply-driven Ghosh system, very similar steps need to be made. Instead of the `input_coefficient_matrix_create()` in this example, use the `output_coefficient_matrix_create()`, and 
instead of creating the Leontief-inverse, create the Ghosh-inverse with `ghosh_inverse_create()`.

### Input coefficients matrix

The `input_coefficient_matrix_create()` function relies on the following equation. The numbering of the equations follows the numbering of the *Eurostat Manual*.
...
```
Let's print the top left corner 5x5 submatrix of the table. The `iotables` objects contain a key column, in this case `prod_na` (which shows the rows following the [Eurostat vocabulary for products and gross value added components](http://dd.eionet.europa.eu/vocabulary/eurostat/prod_na/).

```{r printinputcoeff, echo=FALSE}
head(input_coeff_matrix_cz[1:5,1:6])
```
...
### Creating the Leontief-matrix and its inverse {#leontief-inverse}

The Leontief matrix is derived from Leontief equation system.  

(19) $(I-A)x = y$

The Leontief matrix is defined as $(I-A)$ and it is created with the `leontief_matrix_create()` function.

The Leontief inverse is (I-A)^-1^ and it is created with the `leontief_inverse_create()` function from the Leontief-matrix.

The code chunk below prints a small part of the Czech Leontief-inverse.

```{r leontief}
L_cz <- leontief_matrix_create(input_coeff_matrix_cz)
I_cz <- leontief_inverse_create(input_coeff_matrix_cz)

L_sk <- leontief_matrix_create(input_coeff_matrix_sk)
I_sk <- leontief_inverse_create(input_coeff_matrix_sk)
```

Again, let's print the top left 5x5 submatrix from the Czech Leontief-inverse:

```{r printpartleontief, echo=FALSE}
head(I_cz[1:5,1:6])
```
...
Again, let's print only the top left 5x5 submatrix:

```{r printdirect, echo=FALSE}
head(direct_cz[1:5,1:6])
```
...
The total effects take into consideration that the economy is a system. When an sector faces increased demand and tries to fulfil it, it will create additional demand among its suppliers, and will make more products available for further use in the downstream purchasing sectors.

### Input multipliers

The `input_multipliers_create` function will create the various multipliers for each product. *The calculation follows the Eurostat Manual p 499-502.*
...
input_requirements_cz <- primary_inputs_cz[,names(primary_inputs_cz) %in% names(I_cz)]

multipliers_cz <- input_multipliers_create(
  input_requirements = input_requirements_cz,
  Im = I_cz, 
  multiplier_name = paste0(input_requirements_cz$prod_na, "_multiplier")  ) 

multipliers_sk <- input_multipliers_create( 
  input_requirements = primary_inputs_sk[not_removed_cols], 
  Im = I_sk, 
  multiplier_name = primary_inputs_sk[not_removed_cols]$prod_na) 
```
```{r printtotal, echo=FALSE}
head(multipliers_cz[1:5, 1:6])
```
...
emp_indicator_cz <- full_join (cz_io, 
                               emp_cz) %>% 
  coefficient_matrix_create( return_part = 'primary_inputs') %>%
  filter ( prod_na == "employment_total" )
...
```
The `vector_transpose_longer()` is a wrapper around [tidyr::pivot_longer](https://tidyr.tidyverse.org/reference/pivot_longer.html) but respects the key column which contains the row names. It helps us shape the results in a better readible, long format.


```{r printemploymenteffect}
vector_transpose_longer(emp_effect_cz, 
                        values_to ="employment_effect_cz") %>%
  left_join (vector_transpose_longer(emp_effect_sk, 
                                     values_to = "employment_effect_sk"), 
             by = 'nace_r2' ) %>%
...
Only the top 8 employment multipliers are shown (arranged by the ordering of the Slovak multipliers in descending order):

```{r printemploymentindicator}
vector_transpose_longer (emp_multiplier_cz, values_to ="employment_multiplier_cz") %>%
  left_join(vector_transpose_longer(emp_multiplier_cz, 
                                    values_to = "employment_multiplier_sk"),
            by = 'nace_r2' ) %>%
  mutate(across(starts_with("employmnet"), function(x) x*1000)) %>%
...
Let's take a look at the five top employment multipliers in Czechia:

```{r printoutputmultiplierscz}
vector_transpose_longer (emp_multiplier_cz, 
...
output_coeff_sk <- output_coefficient_matrix_create( 
  ...
  total = "tfu")
```
The top left 5x5 submatrix of the output coefficient matrix is shown below for Czechia:

```{r printoutputcoeff, echo=FALSE}
output_coeff_cz[1:5,1:6]
```
...
The first few forward linkage strength indicators are shown below for Czechia:

```{r printforwardlinkages, echo=FALSE}
head(cz_fw)
```
