...
collect_metadata_60
possible_sources <- c(
  "naio_cp17_r2", "naio_10_cp1700", "naio_10_cp1750",
...
for (i in 1:length(possible_sources)) {
  ...
  if ("stk_flow" %in% names(tmp)) {
    if (is.null(metadata_collector_p)) {
      metadata_collector_p <- distinct(
        tmp, source, stk_flow, induse, prod_na,
      ...
      metadata_collector_p <- rbind(
        metadata_collector_p,
        tmp_narrow
      )
    ...
    if (is.null(metadata_collector_t)) {
      metadata_collector_t <- distinct(
        tmp, source, t_rows2, t_cols2,
        t_rows2_lab, t_cols2_lab
      )
    } else {
      metadata_collector_t <- rbind(
