...
germany_long <- germany %>%
  ...
  mutate(t_cols2_lab = plyr::mapvalues(t_cols2,
    from = c(
      "agriculture_group", "manufacturing_group", "construction_group",
    ...
    to = c(
      "Agriculture group", "Manufacturing group", "Construction group", "Trade group",
  ...
  mutate(
    ordering_c = as.numeric(ordering_c),
  ...
  mutate(t_rows2 = forcats::fct_reorder(
    t_rows2,
    as.numeric(ordering_r)
  )) %>%
  mutate(t_cols2 = forcats::fct_reorder(
    t_cols2,
    as.numeric(ordering_c)
  )) %>%
  mutate(iotables_label_r = forcats::fct_reorder(
    t_rows2,
    as.numeric(ordering_r)
  )) %>%
  mutate(iotables_label_c = forcats::fct_reorder(
    t_cols2,
    as.numeric(ordering_c)
  )) %>%
...
not_used <- function() {
  germany_airpol_long <- germany_airpol %>%
    ...
    pivot_longer(
      cols = -any_of(c("iotables_row", "prod_na")),
      names_to = "iotables_col",
      values_to = "values"
    ) %>%
    mutate(induse = plyr::mapvalues(.data$iotables_col,
      from = c(
        "agriculture_group", "industry_group", "construction_group",
        "trade_group", "business_services_group", "other_services_group",
        "final_consumption_households", "output_bp"
      ),
      to = c(
  ...
  iotable_get(
    labelled_io_data = germany_airpol, source = "germany_1995",
